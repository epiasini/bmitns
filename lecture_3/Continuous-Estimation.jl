...
Std deviation of the stimulus distribution:
``\sigma_s = ``$(@bind σₛ Slider(0.01:0.01:4, default=2, show_value=true))

Number of experimental trials:
``N = ``$(@bind N Slider(1:1:500, default=250, show_value=true))

We can generate N stimuli according to ``s\sim\mathcal{N}(\mu,\sigma_s)`` and display them as a histogram below.
"""
...
```math
\begin{gather*}
\mu_\hat{s}(s) = \frac{J\cdot s+J_s\cdot\mu}{J+J_s} = \frac{(1/\sigma)^2 \cdot s+(1/\sigma_s)^2\cdot\mu}{(1/\sigma)^2+(1/\sigma_s)^2}\\
\sigma_\hat{s}(s) = \frac{J}{J+J_s}\sigma = \frac{1/\sigma}{(1/\sigma)^2+(1/\sigma_s)^2}
\end{gather*}
```
...
begin
	...
	ŝ = μ_ŝ + σ_ŝ .* randn(N)
	
	plot([-8, 8], [-8, 8], label=L"Identity (line where $\hat{s}=s$)")
	plot!(s, ŝ, seriestype=:scatter, label="Behavioral data")
	xlims!(-8,8)
	ylims!(-8,8)
	xlabel!(L"Stimulus $s$")
...
```math
\text{likelihood}_n(\rho) = p_\rho(\hat{s}_n|s_n) = \frac{1}{\sqrt{2\pi\sigma_{\rho}^2}}\exp{\left[-\frac{(\hat{s}-\mu_\rho)^2}{2\sigma_{\rho}^2}\right]}
```
where
```math
\begin{gather*}
...
Since all trials are assumed to be conditionally independent, the likelihood computed from the whole dataset is just the product of the likelihoods of the individual trials:
```math
p_\rho(\hat{s}_1,\ldots,\hat{s}_N|s_1,\ldots,s_N) = p(\hat{s}_1|s_1)p(\hat{s}_2|s_2)\ldots p(\hat{s}_N|s_N) = \prod_{n=1}^N p_\rho(\hat{s}_n|s_n)
```

Now, the idea is that we will fit the only free parameter of our model (``\sigma``) by looking for the value of ``\rho`` that maximises the likelihood. Instead of directly maximising the likelihood however, we will maximise the logarithm of the likelihood ("log-likelihood"), which is mathematically equivalent but computationally convenient.

So the function we'd like to maximise is the log-likelihood
```math
\mathcal{L}(\rho) = \log \prod_{n=1}^N p_\rho(\hat{s}_n|s_n) = \sum_{n=1}^N \log p_\rho(\hat{s}_n|s_n)
```

By using the definition of ``p(\hat{s}|s)`` above, we can rewrite this as
```math
\begin{align}
...
begin
	ρ_range = 0.3:0.01:2.5
	ll = log_likelihood.(ρ_range)
	ρ_best_fit = ρ_range[argmax(ll)]
	plot(
		ρ_range, ll,
	...
	vline!(
		[σ],
		label="True value of sensory noise (σ)",
		linewidth=5)
	vline!(
		[ρ_best_fit],
		label="Best fit value of sensory noise from model",
		linewidth=2)
	scatter!(
		[ρ_best_fit],
		[maximum(ll)],
		markershape=:star6,
		markersize=6,
		label="Maximum value of log-likelihood"
	)
end
...
(note however that the numerical value of the log-likelihood is not particularly meaningful). **The value we found for ``\rho^*`` should be compared with the true value of the sensory noise, which is ``\sigma`` = $(σ)**.
"""
...
manifest_format = "2.0"
project_hash = "a475eb3fb1026a99c54fc0312993be1aff054074"
