...
using PlutoUI, Random, Distributions, Plots, Optim, Printf
...
function simulate_experiment(σ, N)
    ...
    # compute probability pₛ of choosing category ĉ=1 for each stimulus level s in the experiment
    pₛ = @. cdf(Normal(), s/σ)
    
    # generate empirical data: for each level s, sample nₛ choices from a Binomial distribution with probability of "success" (→ choosing ĉ=1) equal to p
    lₛ = @. rand(Binomial(nₛ, pₛ))
...
function plot_psychometric(σ, N, display_behavioral_variance)
    ...
    plot!(s_plot_range,
		cdf(Normal(), s_plot_range/σ̂),
		color="red",
		label="Psychometric curve fit to data",
		legend=:topleft
	)
    ...
    annotate!(
        2, 0.5,
        "\$\\sigma_{{\\mathrm{{true}}}}=$σ\$\n\$\\sigma_{\\mathrm{fit}}=$(@sprintf("%.2f", σ̂))\$")
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "155e715c000a449eddfda857ba3e32b3185a887a"
